...
class SVGP(nn.Module):
    # --- follow spaVAE implementation on SVGP with modification
    def __init__(self, fixed_inducing_points, initial_inducing_points, 
            fixed_gp_params, kernel_type, kernel_scale, jitter, N_train, dtype, device):
        ...
        self.kernel = Kernel(kernel_type=kernel_type, scale=kernel_scale, fixed_scale=fixed_gp_params, dtype=dtype, device=device).to(device)
    ...
    def kernel_matrix(self, x, y, diag_only=False):
        ...
        if diag_only:
            return torch.diagonal(kernel_mat)
        else:
            return kernel_mat
    ...
    def variational_loss(self, x, y, noise, mu_hat, A_hat):
        ...
        K_nn = self.kernel_matrix(x, x, diag_only=True) # (b)
        ...
        # @ Wenjing Ma: use torch.slogdet instead -> more numerical stability
        #_, K_mm_log_det = torch.slogdet(_add_diagonal_jitter(K_mm, self.jitter))
        #_, S_log_det = torch.slogdet(_add_diagonal_jitter(A_hat, self.jitter))
    ...
    def approximate_posterior_params(self, index_points_test, index_points_train=None, y=None, noise=None):
        ...
        K_xx = self.kernel_matrix(index_points_test, index_points_test, diag_only=True)  # (x)
        K_xm = self.kernel_matrix(index_points_test, self.inducing_index_points)  # (x, m)
        K_mx = torch.transpose(K_xm, 0, 1)  # (m, x)

        K_nm = self.kernel_matrix(index_points_train, self.inducing_index_points)  # (N, m)
