...
# set optparse
option_list = list(
    make_option(c("--result_dir"), type='character', default=NULL, help="Result directory."),
    make_option(c("--numCores"), type="integer", default=8, 
        help="Number of cores used to run SPARKX [default %default]",
        metavar="number")
)
...
# load data
adata = read_h5ad(file.path(result_dir, 'adata.h5ad'))
obj = CreateSeuratObject(counts = t(adata$X), meta.data = adata$obs)
# remove ribosomal genes if any
ribosomal_genes = grep("^Rp[sl]", rownames(obj), value=T)
obj =  obj[!rownames(obj) %in% ribosomal_genes, ]
mt_genes = grep("^mt-", rownames(obj), value=T)
...
for (tp in timepoints) {
    ...
    # significant_gene_number = min(significant_gene_number, 3000) # top 3000 genes to avoid too many genes
    SVGs = sparkX$res_mtest[order(sparkX$res_mtest$adjustedPval),][1:significant_gene_number, ]
    write.csv(SVGs, file.path(result_dir, paste0(tp, '_SVG_sparkx.csv')), quote=F)
    ...
    # SVGs$cluster = clusterlabel
    SVGs$cluster = merged
    write.csv(SVGs, file.path(result_dir, paste0(tp, '_SVG_sparkx_clustered_louvain.csv')), quote=F)
}