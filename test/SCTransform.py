# --- implemented in https://github.com/atarashansky/SCTransformPy
...
def SCTransform(adata,min_cells=5,gmean_eps=1,n_genes=2000,n_cells=None,bin_size=500,bw_adjust=3,inplace=True):
    ...
    genes_log_gmean = np.log10(gmean(X,axis=0,eps=gmean_eps)+1) # can have -Inf if 0
    ...
    if n_genes is not None and n_genes < len(genes_step1):
        ...
        genes_log_gmean_step1 = np.log10(gmean(X[cells_step1,:][:,genes_step1],eps=gmean_eps)+1)
